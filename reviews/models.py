...
class Contributor(models.Model):
    ...
    def initialled_name(self):
        first = self.first_names  # Jerome David
        last = self.last_names  # Salinger
        names = first.split()  # ['Jerome','david']
        initials = ''.join([name[0].upper() for name in names])  # ['J','D'] JD
        return f"{last}, {initials}"  # Salinger, JD
...
class Review(models.Model):
    ...
    def __str__(self):
        return "{} - {}".format(self.creator, self.book.title)
